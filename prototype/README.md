...
| Проверить идентификацию через БНСО (Умка/Скаут) | ✅ (PWA + `lookup.py --bnso-serial`) |
| Идентифицировать метку через PWA в браузере | ✅ https://sayr777.github.io/dynamic-iBeacon/server/lookup-pwa/ |
...
## Тестирование через PWA

Для идентификации метки прототипа без Python и без командной строки:

1. Открой в Chrome: **https://sayr777.github.io/dynamic-iBeacon/server/lookup-pwa/**
2. Введи `Major` и `Minor` из лога USB-serial (`[slot N] TagID=42  Major=0xXXXX  Minor=0xXXXX`)
3. Укажи номер слота в поле «Слот ≈» — поиск пройдёт в диапазоне ±100
4. Метка будет найдена, появится TagID и название остановки из справочника

Для тестирования БНСО-канала:
- Раздели «📟 Данные из БНСО», выбери модель, введи ID из телематики
- Умка: `ID = Major × 65536 + Minor` (пример: `550368416` → Major=8397, Minor=62624)
