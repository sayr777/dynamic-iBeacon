...
| **Рекомендация** | пилот/макет | альтернатива для серийного производства | ✅ **промышленный образец** |
...
|---|:---:|:---:|:---:|
| Прототип / макет | ✅ | ✅ | ✅ |
| **Промышленный образец** | — | резерв для серии | ✅ **основной выбор** |

### Обоснование выбора nRF52832 (YJ-16013) для промышленного образца

1. **Потребление идентично nRF52810** (~5 µА) — смена чипа не влияет на ресурс батареи.
2. **Нет пина RESET у nRF52810** — при зависании прошивки во время разработки только цикл питания; у nRF52832 отдельный RESET упрощает отладку.
3. **Референсный чип nRF5 SDK** — все примеры, туториалы, код сообщества написаны под nRF52832; нRF52810 требует правок в части примеров.
4. **Запас Flash/RAM** — 512 KB / 64 KB против 192 KB / 24 KB; при росте прошивки (сенсор, BLE конфигурация) не нужна смена платформы.
5. **S132 + S112** — можно использовать обе SoftDevice; при необходимости добавить BLE-приём (конфигурация тега) — без смены чипа.
6. **NFC** — возможна NFC-провизия TAG_ID + KEY без SWD программатора.
7. **Цена** — разница $0.40–1.40/шт не окупает ограничения nRF52810 на прототипе.
8. **nRF52810** оправдан при серийном производстве 1000+ шт с финализированной прошивкой.
