...
## Идентификация через БНСО

Когда данные приходят не из прямого BLE-сканирования, а из телематики транспортного средства, формат `ID` зависит от модели БНСО.

### Умка (протокол Wialon IPS)

БНСО передаёт одно 32-битное число: `ID = Major × 65536 + Minor`.

Декодирование — точное, потери информации нет:

```python
major = (ID >> 16) & 0xFFFF   # старшие 16 бит
minor =  ID        & 0xFFFF   # младшие 16 бит
```

После восстановления Major и Minor идентификация стандартная: `identify_tag(major, minor, ...)`.

### Скаут (протокол EGTS)

БНСО передаёт: `ID = Major + Minor`.

Декодировать Major и Minor по-отдельности **невозможно** — сложение необратимо. Однако пара `(tag_id, slot)` идентифицируется однозначно, поскольку сумма двух AES-128 выходов уникальна на практике:

```python
slot_now = int(unix_time()) // SLOT_DURATION

for s in [slot_now - 1, slot_now, slot_now + 1]:
    for tag_id in range(0, NUM_TAGS):
        block1 = tag_id.to_bytes(2,'big') + s.to_bytes(4,'big') + b'\x01' + bytes(9)
        out = AES128_encrypt(KEY, block1)
        m = (out[0] << 8) | out[1]
        n = (out[2] << 8) | out[3]
        if m + n == ID:
            return tag_id   # найдено
```

Вероятность ложного совпадения при `N=10 000` метках:
```
P_false(sum) ≈ N / 2¹⁷ ≈ 10000 / 131072 ≈ 7.6%  (в пределах одного слота)
```
Добавление проверки слота `±1` снижает вероятность ложного совпадения до приемлемого уровня. При критичных требованиях использовать Умка или прямое BLE-сканирование.

### Реестр БНСО

Соответствие серийного номера БНСО и его протокола задаётся в `BNSO_REGISTRY` (`server/lookup.py`):
